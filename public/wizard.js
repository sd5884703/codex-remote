...
export const WIZARD = {
  ...
  zh: {
    leadTitle: '外出访问（主路径）',
    leadBody: '通过Tailscale（在自己的设备之间建立加密私有网络），让手机在外面也能连上家里的电脑。家里同一 Wi-Fi 只是附录备用，不是主成功路径。请记住：先装好 Tailscale 再开服务。',
    enableTs: '启用外出访问：Tailscale',
    skipWarn: '你跳过了 Tailscale。出门不可用。手机只能在家里同一 Wi-Fi 下使用（见附录）。',
    tsTitle: 'Tailscale',
    tsIntro: '我们无法替你完成第三方登录。请按下面步骤，在电脑和手机上用同一个账号自行安装并登录。请先完成 Tailscale，再安装登录后自动启动。',
    tsStepsMac: [
      '在 Mac 上：打开官方下载页或 App Store 安装 Tailscale，打开应用并登录。',
      '在手机上：安装 Tailscale，用【同一个账号】登录。',
      '确认两端在 Tailscale 应用里都显示已连接 / 在线。',
      '记下本机的Tailscale 地址（以 100. 开头的 IP，或魔术域名）。手机要保存的是这个，不是家里 Wi-Fi 的 192.168 地址。',
    ...
    tsStepsWin: [
      ...
      '在手机上：安装 Tailscale，用【同一个账号】登录。',
      '确认两端在 Tailscale 应用里都显示已连接 / 在线。',
      '记下电脑的Tailscale 地址（以 100. 开头的 IP，或魔术域名）。手机要保存的是这个，不是家里 Wi-Fi 的 192.168 地址。',
      '下一步再安装「登录后自动启动」（先装好 Tailscale 再开服务）。会创建用户级计划任务（Scheduled Task），并允许经 Tailscale 访问本机端口。',
    ],
    detectOk: '已检测到本机可能已安装Tailscale或已有Tailscale IP。',
    detectNo: '尚未检测到 Tailscale。请先安装并登录，再点「我已装好，下一步」。',
    nextBtn: '我已装好，下一步',
    alwaysOnTitle: '登录后自动启动',
    alwaysOnBodyMac: '安装用户级登录启动项（launchd），登录 Mac 后自动启动编码遥控。外出路径会同时允许经 Tailscale 访问本机端口，以便Tailscale能连上。',
    alwaysOnBodyWin: '安装用户级计划任务（Scheduled Task），登录 Windows 后自动启动编码遥控。外出路径会同时允许经 Tailscale 访问本机端口，以便Tailscale能连上。',
    alwaysOnBodyLinux: '测试环境：安装轻量保活（记录进程号并后台挂起）。不是给普通用户装的发行包装。',
    meshFirstNote: '先装好 Tailscale 再开服务：请确认Tailscale已在线后，再安装登录后自动启动。',
    ...
    summaryNoMesh: '尚未读到Tailscale 地址：请在电脑的 Tailscale 里查看 100.x IP 或魔术 DNS，在手机里手动填写稳定地址（例如 http://100.x.x.x:8787），确认Tailscale 在线后也可刷新本页。',
