...
export const WIZARD = {
  en: {
    ...
    alwaysOnBodyMac: 'Install a user launchd agent so CodexRemote starts when you log into the Mac. For the Tailscale path it also enables listening on all interfaces (exposeLan) so the phone can reach the port via the mesh.',
    alwaysOnBodyWin: 'Install a user Scheduled Task so CodexRemote starts when you log into Windows. For the Tailscale path it also enables listening on all interfaces (exposeLan) so the phone can reach the port via the mesh.',
    alwaysOnBodyLinux: 'Box QA only: lightweight keepalive (pidfile + nohup). Not distro packaging.',
  ...
  zh: {
    ...
    tsStepsWin: [
      ...
      '在手机上：安装 Tailscale，用【同一个账号】登录。',
      '确认两端在 Tailscale 应用里都显示已连接 / 在线。',
      '记下电脑的组网地址（以 100. 开头的 IP，或魔术 DNS 名称）——手机要保存的是这个，不是家里 Wi-Fi 的 192.168 地址。',
    ...
    nextBtn: '我已装好，下一步',
    alwaysOnTitle: '登录后常开',
    alwaysOnBodyMac: '安装用户级登录启动项（launchd），登录 Mac 后自动启动编码遥控。出门路径会同时打开对外监听（exposeLan），以便私人组网（Tailscale）能访问端口。',
    alwaysOnBodyWin: '安装用户级计划任务（Scheduled Task），登录 Windows 后自动启动编码遥控。出门路径会同时打开对外监听（exposeLan），以便私人组网（Tailscale）能访问端口。',
    alwaysOnBodyLinux: '箱测 QA：安装轻量 keepalive（pidfile + nohup）。不是发行版包装。',
